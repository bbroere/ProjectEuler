...
function _basicPokerScore(hand: Card[]): number[] {
...
/**
 * Determines the winner of a poker game (1 = lhs, 2=rhs)
 */
export function pokerWinner(game: [Card[], Card[]]): number {
    const [score1, score2]: number[][] = game.map(_basicPokerScore);
    for (let i: number = 0; i < score1.length; i++) {
        if (score1[i] > score2[i]) {
            return 1;
        } else if (score1[i] < score2[i]) {
            return 2;
        } // else continue (tie on this level)
    }
    assert(false, "inconclusive scoring");
}

/**
 * Tests the poker implementation
 */
export function testPokerImplementation(): void {
    function _testOutcome(gameString: string, expected: number): void {
        const cards: Card[] = gameString.split(" ").map((t: string): Card => new Card(t));
        assert(pokerWinner([cards.slice(0, 5), cards.slice(5)]) === expected);
    }

    _testOutcome("5H 5C 6S 7S KD 2C 3S 8S 8D TD", 2);
    _testOutcome("5D 8C 9S JS AC 2C 5C 7D 8S QH", 1);
    _testOutcome("2D 9C AS AH AC 3D 6D 7D TD QD", 2);
    _testOutcome("4D 6S 9H QH QC 3D 6D 7H QD QS", 1);
    _testOutcome("2H 2D 4C 4D 4S 3C 3D 3S 9S 9D", 1);
}
